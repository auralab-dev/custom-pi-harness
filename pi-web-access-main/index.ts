...
import { storeDownloadedArtifact } from "./download-storage.ts";
...
type ToolSchemaMode = "full" | "minimal";
...
function getToolSchemaMode(config: WebSearchConfig): ToolSchemaMode {
	return config.toolSchemaMode === "minimal" ? "minimal" : "full";
}
...
export default function (pi: ExtensionAPI) {
	...
	const sourceCheckEnabled = minimalToolSchemas ? false : isToolEnabled(initConfig, "sourceCheck");
	const fetchContentEnabled = isToolEnabled(initConfig, "fetchContent");
	const getSearchContentEnabled = minimalToolSchemas ? false : isToolEnabled(initConfig, "getSearchContent");
	...
	if (webSearchEnabled) pi.registerTool({
		...
		label: "Web Search",
		description: minimalToolSchemas
			? "Search the web. Returns source passages and URLs; use download_file when a source needs local searching."
		...
		promptSnippet: minimalToolSchemas
			? "Search the web; download a result only when local content search is needed."
			: "Use for web research questions. Prefer {queries:[...]} with 2-4 varied angles over a single query for broader coverage. Omit provider unless explicitly overriding the configured default.",
		parameters: minimalToolSchemas
	...
	if (fetchContentEnabled) pi.registerTool({
		...
		label: "Download File",
		description: minimalToolSchemas
			? "Download and extract one URL to a project-local searchable file. Returns only the project-relative path; pass it to file_content_search."
		...
		promptSnippet: minimalToolSchemas
			? "Download a URL to a local searchable file, then use file_content_search on the returned path."
		...
		parameters: minimalToolSchemas
			? Type.Object({ url: Type.String({ description: "URL returned by web_search or otherwise known." }) })
			: Type.Object({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx): Promise<AgentToolResult<Record<string, unknown>>> {
			if (minimalToolSchemas) {
				...
				onUpdate?.({ content: [{ type: "text", text: "Downloading..." }], details: { phase: "fetch", progress: 0 } });
				const results = await runWithProxy(undefined, () => fetchAllContent([url], signal, withRegisteredFetchOptions(undefined, registeredToolNames)));
				const result = results[0];
				if (!result || result.error) throw new Error(result?.error || "Download failed");
				const artifact = storeDownloadedArtifact(ctx.cwd, generateId(), results);
				return {
					content: [{ type: "text", text: artifact.path }],
					details: { path: artifact.path, files: artifact.files, totalChars: artifact.chars, totalBytes: artifact.bytes, sourceUrl: url },
				};
			}
		...
		renderCall(args, theme) {
			...
			if (urlList.length === 0) {
				return new Text(theme.fg("toolTitle", theme.bold("download ")) + theme.fg("error", "(no URL)"), 0, 0);
			}
			const lines: string[] = [];
			if (urlList.length === 1) {
				const display = urlList[0].length > 60 ? urlList[0].slice(0, 57) + "..." : urlList[0];
				lines.push(theme.fg("toolTitle", theme.bold("download ")) + theme.fg("accent", display));
			} else {
				lines.push(theme.fg("toolTitle", theme.bold("download ")) + theme.fg("accent", `${urlList.length} URLs`));
